...
function initEvents() {
    ...
    DOM.quizList.onclick = (e) => {
        ...
        if (countOpt) {
            ...
            // 同步更新設定選單中的選中狀態
            document.querySelectorAll('#count-grid .theme-option').forEach(o => {
                o.classList.toggle('active', parseInt(o.dataset.count) === State.quizCount);
            });
            UI.renderQuizList();
            return;
        }
...
function initSettings() {
    ...
    document.querySelectorAll('#theme-grid .theme-option').forEach(el => {
        ...
        el.onclick = () => {
            ...
            document.querySelectorAll('#theme-grid .theme-option').forEach(o => o.classList.toggle('active', o === el));
        };
    });
