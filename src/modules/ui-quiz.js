...
export const UIQuiz = {
    renderQuizList: () => {
        ...
        const counts = [10, 20, 50, 999];
        let html = `
            <div class="mb-3">
                <div class="flex-between">
                    <div class="challenge-title">挑戰</div>
                    <div class="count-selector-inline">
                        ${counts.map(c => `<span class="count-opt ${State.quizCount === c ? 'active' : ''}" data-count="${c}">${c === 999 ? '全部' : c + '題'}</span>`).join('')}
                    </div>
                </div>
                <div class="grid-2">
                    <div class="quiz-card challenge-card" data-type="comp">
                        <div class="name">🔥 綜合測驗</div><div class="meta">隨機 ${State.quizCount === 999 ? '全部' : State.quizCount + ' 題'}</div>
                    </div>
                    ${wrongIds.length ? `
